...
def init_db():
    ...
    ensure_user('coach', 'Coach123!', 'Ishan', 'coach', 'Your expert coach. Message anytime.')
...
def normalize_badge_icon(icon_value):
    """Convert legacy badge text identifiers into emoji icons."""
    if not icon_value:
        return '🏆'
    icon_key = str(icon_value).strip().lower()
    mapping = {
        'pro': '⚡',
        'legendary': '🔥',
        'champion': '🏆',
    ...
    return mapping.get(icon_key, icon_value)
...
def admin():
    ...
    conn.close()
    return render_template('admin.html', username=session.get('username'), current_user=current_user, users=users)
...
def admin_delete_user(user_id):
    ...
    conn = get_db_connection()
    current_user = conn.execute('SELECT * FROM users WHERE id = ?', (current_user_id,)).fetchone()
    if not current_user or current_user['role'] != 'admin' or current_user_id == user_id:
        conn.close()
...
def social():
    ...
    conn.close()
    return render_template('social.html', username=session.get('username'), users=users, current_user=current_user)


@app.route('/social/<int:user_id>')
...
def chat_with(recipient_id):
    ...
    user_id = session['user_id']
    conn = get_db_connection()
    recipient = conn.execute('SELECT id, username, display_name, role FROM users WHERE id = ? AND role != ?', (recipient_id, 'admin')).fetchone()
    if not recipient:
        conn.close()
...
@app.route('/chat/<int:recipient_id>/delete/<int:message_id>', methods=['POST'])
def unsend_message(recipient_id, message_id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    user_id = session['user_id']
    conn = get_db_connection()
    ...
    if message and message['sender_id'] == user_id and message['recipient_id'] == recipient_id:
...
def badges():
    ...
    for achievement in achievements:
        achievement['icon_emoji'] = normalize_badge_icon(achievement.get('icon_emoji'))
...
@app.route('/profile/<int:target_user_id>', methods=['GET', 'POST'])
def profile(target_user_id=None):
    ...
    if current_user['role'] == 'admin' and not target_user_id:
        conn.close()
        return redirect(url_for('admin'))

    edit_user_id = target_user_id if target_user_id and current_user['role'] == 'admin' else current_user_id
    user = conn.execute('SELECT * FROM users WHERE id = ?', (edit_user_id,)).fetchone()
    if not user:
        conn.close()
        return redirect(url_for('profile'))

    achievements = conn.execute(
        'SELECT id, badge_name, description, icon_emoji, unlocked_at FROM achievements WHERE user_id = ? ORDER BY unlocked_at DESC',
        (edit_user_id,)
    ).fetchall()
    achievements = [dict(a) for a in achievements]
    for achievement in achievements:
    ...
    if request.method == 'POST':
        ...
        if not username_input:
            ...
            conn.close()
            return redirect(url_for('profile', target_user_id=edit_user_id) if edit_user_id != current_user_id else url_for('profile'))

        username = normalize_username(username_input)
        if username.lower() != user['username'].lower():
            existing_user = conn.execute(
                'SELECT id FROM users WHERE LOWER(username) = ? AND id != ?',
                (username.lower(), edit_user_id)
            ).fetchone()
            if existing_user:
                flash('Username is already taken.', 'danger')
                conn.close()
        ...
        if avatar and avatar.filename:
            ...
            ext = filename.rsplit('.', 1)[1].lower()
            new_name = f'user_{edit_user_id}_{int(time.time())}.{ext}'
        ...
        conn.execute('UPDATE users SET username = ?, display_name = ?, bio = ?, avatar_path = ?, password_hash = ? WHERE id = ?',
                     (username, display_name, bio, avatar_filename, password_hash, edit_user_id))
        conn.commit()
        conn.close()
    ...
    conn.close()
    return render_template('profile.html', user=user, current_user=current_user, achievements=achievements, edit_user_id=edit_user_id)
